...
def get_logger(name, log_dir, rank=-1):
    '''
    Args:
        name(str): name of logger
        log_dir(str): path of log
        rank(int): rank in distributed training, -1 for non-distributed
    '''
    if rank == -1 or rank == 0:
...
def get_scheduler(config, optimizer):
    assert config.sch in ['StepLR', 'MultiStepLR', 'ExponentialLR', 'CosineAnnealingLR', 'ReduceLROnPlateau',
                        'CosineAnnealingWarmRestarts', 'WP_MultiStepLR', 'WP_CosineLR', 'ConstantLR'], 'Unsupported scheduler!'
    if config.sch == 'StepLR':
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_func)
    elif config.sch == 'ConstantLR':
        lr_func = lambda epoch: 1.0
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_func)
...
def save_imgs(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None):
    ...
    plt.subplot(4, 1, 4)
    plt.imshow(img)
    plt.imshow(att_msk, cmap='jet', alpha=0.5) 
    plt.axis('off')
    ...
    def __init__(self, data_name, train=True):
        if data_name == 'isic18':
            ...
            if train:
                ...
                self.mean = 149.8485
                self.std = 35.3346
        elif data_name == 'busi' or data_name == 'bus':
            if train:
                self.mean = 159.922
                self.std = 28.871
            else:
            ...
            if train:
                ...
                self.mean = [0.485, 0.456, 0.406]
                self.std = [0.229, 0.224, 0.225]
            print(f"[WARNING] Unknown dataset '{data_name}'. Using default ImageNet normalization parameters.")
