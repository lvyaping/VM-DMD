...
class setting_config:
    ...
    relational_kd = {
        ...
        'stage_weights': [0.5,0.75,1.0,1.25],
        'log_dir_weights': False,
        'teacher': {
            'type': 'swin',
            'repo_path': './Swin-Transformer-Semantic-Segmentation',
            'checkpoint': './Swin-Transformer-Semantic-Segmentation/pretrained/BUS.pth',
            'model_kwargs': {
            ...
            'attention_heads_config': {}
        }
    }

    datasets = 'bus' 
    if datasets == 'busi':
        data_path = './data/Breast_ultrasound/'
    elif datasets == 'bus':
        data_path = './data/Breast_BUS/'
    else:
    ...
    img_save_path = 'test_results/'

    # Compatible with old configuration (replaced by init_stages_from_swin)
    init_stage0_from_swin = False
    init_stage0_swin_ckpt = './Swin-Transformer-Semantic-Segmentation/pretrained/BUS.pth'
    init_stage3_from_swin = False
    init_stage3_swin_ckpt = init_stage0_swin_ckpt

    # New multi-Stage initialization configuration (adjust as needed)
    init_stages_from_swin = []
    init_stages_swin_ckpt = init_stage0_swin_ckpt
    init_stages_use_projection = True 
