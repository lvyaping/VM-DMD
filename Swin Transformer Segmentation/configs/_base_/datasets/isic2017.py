# dataset settings
dataset_type = 'ISIC2017Dataset'  # Use custom dataset class with label conversion support
data_root = '/home/lvyp/ISIC2017/'
img_norm_cfg = dict(
...
train_pipeline = [
    ...
    # Key fix: convert label values from 0/255 to 0/1
    # ISIC2017 labels are 0 (background) and 255 (lesion), need to convert to 0 and 1
    dict(type='ConvertLabels', label_mapping={0: 0, 255: 1}, ignore_index=255),
...
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=2,  # Reduce number of workers to lower temporary file requirements (insufficient disk space)
    train=dict(
        type=dataset_type,
        data_root=data_root,
        img_dir='train/image',
    ...
    val=dict(
        ...
        # test_mode will be forced to True in train.py, so no need to set it here
        classes=('background', 'lesion'),
        palette=[[0, 0, 0], [255, 255, 255]]),
    test=dict(
        type=dataset_type,
